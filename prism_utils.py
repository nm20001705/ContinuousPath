# prism_utils.py (working – no extra clustering, single wire per rib)
import FreeCAD
import Part
...
def create_bridges_trimmed_to_wing(data_by_rib, rib_center_lines, plane_normal,
    ...
    for idx, data in data_by_rib.items():
        ...
        for i in range(len(pts_extended)-1):
            ...
            wire1 = Part.makePolygon(v1+[v1[0]]); faces.append(Part.Face(wire1))
            wire2 = Part.makePolygon(v2+[v2[0]]); faces.append(Part.Face(wire2))
            try:
