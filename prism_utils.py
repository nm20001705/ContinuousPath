...
def create_bridges_trimmed_to_wing(data_by_rib, rib_center_lines, plane_normal,
    ...
    Handles multiple disjoint wire pieces per rib via clustering.
    """
    if doc is None:
    ...
    for idx, data in data_by_rib.items():
        ...
        # Cluster points based on XY proximity (threshold 5.0 mm)
        clusters = []
        if pts_sorted:
            current = [pts_sorted[0]]
            for p in pts_sorted[1:]:
                if current[-1].distanceToPoint(p) < 5.0:
                    current.append(p)
                else:
        ...
        for cluster in clusters:
            if len(cluster) < 2:
                continue
            pts_extended = extend_rib_wire(cluster, extend_length)
            seg_prisms = []
            for i in range(len(pts_extended)-1):
                p1 = pts_extended[i]
                p2 = pts_extended[i+1]
                offs = [( half_w,  half_h), ( half_w, -half_h), (-half_w, -half_h), (-half_w,  half_h)]
    ...
    if all_bridges:
        ...
        doc.recompute()
        print(f"Created {len(all_bridges)} trimmed bridge pieces.")
        return obj
    else:
